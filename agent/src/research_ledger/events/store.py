...
class ResearchEventStore:
    ...
    def _validate_external_snapshot_evidence_v3(
        ...
        if len(references) != 1:
            raise EventValidationError(
                "snapshot evidence requires one producer artifact"
            )
        reference = references[0]
        try:
            from src.alpha_quality.decision_v2.evidence_v3 import (
            ...
            expected = DecisionSnapshotEvidenceServiceV3.event_payload(
                DecisionSnapshotEvidenceServiceV3.evidence_id(
                    record.evidence_hash
                ),
                record,
                reference,
            ...
            raise EventValidationError(
                "snapshot evidence artifact or source replay is invalid"
            ) from exc
        if record.evidence_kind != "snapshot" or rebuilt != record:
            raise EventValidationError(
                "snapshot evidence differs from deterministic source replay"
            )
        if dict(payload) != expected:
            raise EventValidationError(
    ...
    def _validate_external_research_dossier(
        ...
        if event_type not in {
            "ResearchDossierRecorded", "ExperimentRunReportRecorded",
            "ResearchReleaseManifestRecorded", "ResearchDossierMaterializationFailed",
        }:
            return
        try:
            ...
            if event_type == "ResearchDossierMaterializationFailed":
                ...
                if (
                    payload["failure_id"] != expected_id
                    or payload["decision_effect"] != "none"
                ):
            ...
            if event_type == "ResearchDossierRecorded":
                candidate_refs = [
                    ref for ref in payload["artifact_refs"]
                    if ref["media_type"] == CANDIDATE_DOSSIER_MEDIA_TYPE
                ]
                view_refs = [
                    ref for ref in payload["artifact_refs"]
                    if ref["media_type"] == AUDIENCE_VIEW_MEDIA_TYPE
                ]
                ...
                views = [
                    artifacts.read(
                        ...
                        semantic_hash=str(
                            _strict_json(
                                self.artifact_root.joinpath(
                                    *str(ref["relative_path"]).split("/")
                                )
                            )["view_hash"]
                        ),
            ...
            if event_type == "ExperimentRunReportRecorded":
                ...
                if (
                    ...
                    != report.payload["candidate_dossier_hashes"]
    ...
    def _validate_transition(
            ...
            if (
                ...
                or (
                    ...
                    and (
                        ...
                        or json.loads(str(decision["payload"]))["factor_spec_id"]
                        != payload["factor_spec_id"]
                    )
                )
            ):
        ...
        if event_type == "SnapshotDecisionEvidenceV3Recorded":
            if draft.run_id != payload["evidence_run_id"]:
                raise EventTransitionError(
                    "snapshot evidence event run differs from producer run"
                )
            if self._tail_hash(conn) != payload["source_watermark_event_hash"]:
            ...
            if (
                ...
                raise EventTransitionError(
                    "snapshot evidence source identity or order differs"
                )
            prior = conn.execute(
                """
                SELECT 1 FROM research_events
            ...
            if prior is not None:
                raise EventTransitionError(
                    "snapshot evidence already exists for factor and run"
                )
            return
    ...
    def _verify_references_and_lifecycle(events: list[ResearchEventEnvelope]) -> bool:
        ...
        for event in events:
            ...
            if event.event_type == "FactorDefinitionRecorded":
                ...
                key = (event.run_id, str(payload["factor_spec_id"]))
                terminal = events_by_hash.get(
                    str(payload["terminal_dossier_event_hash"])
                )
                decision_hash = payload["quality_decision_event_hash"]
                decision = (
                    None if decision_hash is None
                    else events_by_hash.get(str(decision_hash))
                )
                sources = [
                    events_by_hash.get(str(item))
                    for item in payload["source_event_hashes"]
                ...
                if (
                    ...
                    or (
                        ...
                        and (
                            ...
                            or decision.payload["factor_spec_id"]
                            != payload["factor_spec_id"]
                        )
                    )
                ):
                ...
                prior_sources = [
                    item for item in events[:event_order[event.event_hash]]
                ...
                if (
                    ...
                    != tuple(sorted(item.event_hash for item in prior_sources))
                ):
                    return False
